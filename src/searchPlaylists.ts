 ...
 */
export const extractPlaylistsFromSearchResponse = (body: any, onlyOfficialPlaylists: boolean): PlaylistPreview[] => {
    try {
        const contents =
...
export async function searchForPlaylists(
    ...
    try {
        return extractPlaylistsFromSearchResponse(response.data, options?.onlyOfficialPlaylists ?? false);
    } catch (e) {
        console.error(e);
        return [];
    }
